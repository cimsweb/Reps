...
Детальный чеклист — в [tasks/mvp-0.md](tasks/mvp-0.md). Текущая разработка — **MVP 3** ([tasks/mvp-3.md](tasks/mvp-3.md)). Общие правила — в [AGENT.md](AGENT.md).

API: [аутентификация](docs/api/auth.md) · [coaching MVP 1](docs/api/coaching.md) · [training MVP 2](docs/api/training.md) · [схема БД coaching](docs/db/coaching-schema.md) · [схема БД training](docs/db/training-schema.md)
...
- **Тренер** (`/coach`): отправка приглашения, список приглашений со статусами, список связанных спортсменов, карточка `/coach/athletes/{id}` (профиль, рекорды, feedback).
- **Спортсмен** (`/athlete`): принятие/отклонение приглашений, профиль, CRUD рекордов, обратная связь по тренировке.

Типовой сценарий: тренер приглашает → спортсмен принимает → спортсмен заполняет профиль и оставляет feedback → тренер открывает карточку спортсмена.

### Training UI (MVP 2)

- **Тренер:** карточка спортсмена → «Создать план» (`/coach/athletes/{id}/training/create`), просмотр плана и итогов за неделю/месяц.
- **Спортсмен:** план тренировок, отчёт 0–10 (трудность + эмоции), свои итоги за период.

Типовой сценарий: тренер создаёт план на день/неделю → спортсмен видит план и отправляет отчёт → тренер смотрит итоги на карточке.
...
| MVP 2     | Планы тренировок и отчёты | После курса | [tasks/mvp-2.md](tasks/mvp-2.md) |
| **MVP 3** | Интеграции и экосистема   | **Текущий** | [tasks/mvp-3.md](tasks/mvp-3.md) |

### MVP 1 — Связка тренер ↔ спортсмен

Приглашения по email, обратная связь по тренировкам, профиль спортсмена, личные рекорды, ручные ссылки на Garmin Connect в отчётах о тренировках.

### MVP 2 — Планы тренировок и отчёты

Тренер составляет план (день / неделя): бег, велосипед, зал, циклы и списки упражнений. Спортсмен сдаёт отчёт с оценкой трудности и эмоций (0–10). Просмотр плана и итогов за неделю и месяц.

### MVP 3 — Интеграции и экосистема

Интеграция с Garmin API отключена (см. миграцию `011`). Ручные ссылки на активности Garmin Connect остаются в отчётах о тренировках.
